...
class Joueur:
    ...
    def valider_joueur(self, nom, no_joueur, gobelets):
        ...
        """
        if not (type(nom) is str and nom != ""):
            raise GobbletError("Le nom du joueur doit être une chaine de caractères non vide.")
        
        if not (type(no_joueur) is int and 1 <= no_joueur <= 2): 
            raise GobbletError("Le numéro du joueur doit être 1 ou 2.")

        if not (type(gobelets) is list):
            raise GobbletError(f"Les piles de gobelets doivent être spécifiés sous la forme d'une liste. {gobelets}")
        ...
        for pile in gobelets:
            ...
            if len(pile) == 2:
                if type(pile[0]) is not int and type(pile[1]) is not int:
                    raise GobbletError("Une pile doit être une liste de deux entiers ou une liste vide.")
        gobjects = list(map(lambda g: Gobblet(g[1], no_joueur) if g != None else None, gobelets))
    ...
    def récupérer_le_coup(self, plateau):
        ...
        if type(origine) != int and (type(origine) != list and len(origine) != 2):
        ...
        if type(destination) != list and len(destination) != 2:
